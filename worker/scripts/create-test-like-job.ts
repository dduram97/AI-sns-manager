/**
 * Dev/verify helper: insert a planned like action_job for CDP Worker tests.
 *
 * Usage:
 *   npm run test:create-like -- --url=https://m.blog.naver.com/{blogId}/{logNo}
 *   WORKER_TEST_LIKE_URL=https://... npm run test:create-like
 *   WORKER_TEST_POST_URL=https://... npm run test:create-like
 *
 * Optional:
...
function resolveTargetUrl(): string {
  ...
  const fromEnv =
    process.env.WORKER_TEST_LIKE_URL?.trim() ||
  ...
  if (!url) {
    throw new Error(
      "target_url required. Example:\n" +
        "  npm run test:create-like -- --url=https://m.blog.naver.com/blogId/123\n" +
        "  WORKER_TEST_LIKE_URL=https://... npm run test:create-like",
    );
  }
  try {
  ...
  if (!/blog\.naver\.com\/[^/?#]+\/\d+/i.test(url) && !/[?&]logNo=\d+/i.test(url)) {
    throw new Error(
      `like job needs a post URL (blogId/logNo), got: ${url}`,
    );
  }
  return url;
}
...
async function resolvePersonAndWorkflow(
  ...
  const { data: person, error: personErr } = await db
    .from("persons")
    .insert({
      display_name: `[cdp-worker:test-like] ${stamp}`,
      discover_meta: {
        worker_test: true,
        purpose: "cdp_worker_like",
...
async function main() {
  ...
  console.info("[test:create-like] created", {
    jobId: job.id,

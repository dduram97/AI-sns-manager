/**
 * Structured action_job failure payload (worker → target_ref.execution_failure).
 * Kept dependency-free so admin UI / scripts can mirror the shape.
 */
...
export type ActionFailureDetail = {
  ...
  /** Ordered step trail for like/comment/neighbor */
  steps?: string[];
  retryable: boolean;
};

export type ActionExecuteFailure = {
  ok: false;
  jobId: string;
  /** Human-readable (also written to action_jobs.error) */
  error: string;
...
/** Map common free-text worker errors into structured codes (fallback). */
export function classifyWorkerErrorText(
  ...
  if (/comment input|fill did not stick/i.test(msg)) {
    return makeFailure({
  ...
  if (/submit button not found|comment submit/i.test(lower)) {
    return makeFailure({
